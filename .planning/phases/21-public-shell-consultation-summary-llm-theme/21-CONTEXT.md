...
- **D-14:** Guardrail regex: runs in the Inngest function POST-generation (after the LLM returns text, before the JSONB write). If a stakeholder name pattern matches, that section is stored with `status: 'blocked', error: 'guardrail-violation'`. Moderator sees red badge + the matched pattern. **No public render** for blocked sections. The function does NOT throw — other sections proceed normally. Pattern source: live `users.name` lookup at execution time (single column per actual schema — split on whitespace, drop tokens shorter than 4 chars to avoid false-positive matches on common words), joined with a small static list of common PII patterns (email, phone, capital-letter `FirstName LastName` pairs). Per Inngest serialization constraint (Phase 17 Buffer pitfall): the guardrail step returns `regex.source` as a STRING; per-section steps reconstruct `new RegExp(source, 'i')` locally because RegExp objects do not survive JSON serialization across `step.run` boundaries.
